...
def read_shapefile(path):
    ...
    return features
    # keys = ['type', 'geometry', 'properties', 'id']

def get_bounds(shapes,name=None):
    if name == 'all':
        xs = []
        ys = []
        for shape in shapes:
            xs += [i[0] for i in shape['geometry']['coordinates'][0]]
            ys += [i[1] for i in shape['geometry']['coordinates'][0]]
            new_xs = []
            new_ys = []
        for x in xs:
        ...
        if shape['geometry']['type'] == 'Polygon':
            ...
            xs = []
            ys = []
            for s in shape['geometry']['coordinates']:
                xs += [i[0] for i in s[0]]
                ys += [i[1] for i in s[0]]
        return [min(xs),min(ys),max(xs),max(ys)]

def write_shapefile(name,data,properties={}):
    w = shapefile.Writer(name)
    if len(properties.keys()) == {}:
        ...
        for key in properties.keys():
            w.field(key,'C','100')
        w.record(*properties.values())
    w.poly(data)
    w.close()
    # 切城市shape
    # shapes = read_shapefile('2+26+11_new城市边界/2+26+11_new城市边界.shp')
    # for shape in tqdm(shapes):
    #   if shape['geometry']['type'] != 'MultiPolygon':
    #      write_shapefile('city_shape/'+shape['properties']['Name'],shape['geometry']['coordinates'],shape['properties'])
    #   else:
    #      coordinates = []
    #      for s in shape['geometry']['coordinates']:
    #        coordinates.append(s[0])
    #      write_shapefile('city_shape/'+shape['properties']['Name'],coordinates,shape['properties'])

def write_shapefiles(name,datas,property_list):
    w = shapefile.Writer(name)
    for key in property_list[0].keys():
        w.field(key,'C','100')
    for data,properties in zip(datas,property_list):
        w.poly(data)
        w.record(*properties.values())
    w.close()
    # shapes = read_shapefile('2+26+11_new城市边界/2+26+11_new城市边界.shp')
    ...
    # for shape in tqdm(shapes):
    #   if shape['geometry']['type'] != 'MultiPolygon':
    #      datas.append(shape['geometry']['coordinates'])
    ...
    #      properties.append(shape['properties'])
    # write_shapefiles('33城市',datas,properties)

def convertshape_3D_2D(shps):
    if np.array(shps[0]['geometry']['coordinates']).shape[2] != 3:
        return shps
    for shp in shps:
        if shp['geometry']['type'] == 'Polygon':
            for i in range(len(shp['geometry']['coordinates'][0])):
                shp['geometry']['coordinates'][0][i] = [shp['geometry']['coordinates'][0][i][0],shp['geometry']['coordinates'][0][i][1]]
        elif shp['geometry']['type'] == 'MultiPolygon':
            for part in shp['geometry']['coordinates']:
                for i in range(len(part)):
                    part[i] = [part[i][0],part[i][1]]
    return shps
...
def get_colorbar(data,c_type):
    # if data.min() == 0:
    #   l = list(set(list(data.reshape(data.shape[0]*data.shape[1]))))
    #   l.sort()
    #   _min = l[1]
    #   _max = data.max()
    # else:
    ...
    if c_type == 'HCHO-NO2':
        ...
        boundaries = [1, 2, 4, 6, data.max()]
        colors = [(0/255,192/255,255/255),(127/255,255/255,128/255),(255/255,192/255,0/255),(255/255,0/255,0/255)]
        cmap = mpl.colors.ListedColormap(colors)
        norm = mpl.colors.BoundaryNorm(boundaries, cmap.N, clip=True)
    
    elif c_type == 'O3':
        step = (_max-_min)/5
        boundaries = [_min + i*step for i in range(5)]
        colors = [(255/255,0/255,0/255),(0/255,0/255,255/255)]
    

    # norm = mpl.colors.Normalize(np.min(boundaries), np.max(boundaries))

    # clrs = [[norm(boundaries[0]), colors[0]]]
    # for i in range(len(colors)-1):
    #   clrs.append([norm(boundaries[i+1]),colors[i+1]])
    # clrs.append([1.0, colors[-1]])
    # cmap = mpl.colors.LinearSegmentedColormap.from_list('cmap', clrs, 600)


    return cmap, norm

def output_colorbar(path,_type):
    fig = plt.figure(figsize=(3, 8))
    ax3 = fig.add_axes([0.3, 0.2, 0.1, 0.3])
    if _type == 'HCHO-NO2':
        boundaries = [0, 1, 2, 4, 6]
        colors = [(0/255,0/255,225/255), (0/255,192/255,255/255),(127/255,255/255,128/255),(255/255,192/255,0/255),(255/255,0/255,0/255)]
        cmap = mpl.colors.ListedColormap(colors)
        norm = mpl.colors.BoundaryNorm(boundaries, cmap.N, clip=True)
    elif _type == 'O3':
        ticks = [0,0.14]
    ...
    boundaries =  [30, 50, 75, 115, 150, 200]
...
def csv_to_shp(csv_path,shp_path):
    ...
    datas['geometry']=datas.apply(lambda x:Point(x['经度'], x['纬度']),axis=1)
    gdf = gpd.GeoDataFrame(datas)
    gdf.fillna(value=0, inplace=True)
